...
const TaskForm = ({ defaultProgressOrder, type }: TaskFormProps): JSX.Element => {
  const [title, setTitle] = useState<string>('')
  const [detail, setDetail] = useState<string>('')
  const [dueDate, setDueDate] = useState<string>('')
  const [progressOrder, setProgressOrder] = useState<number>(defaultProgressOrder)

  const {addingTaskHandler} = useTaskHandlers()
  ...
  function submitHandler() {
    if(type === TASK_MODAL_TYPE.CREATE) {
      addingTaskHandler({title, detail, dueDate, progressOrder, id: Math.random()})
    }
    clearInput()
  }
  ...
  return (
    <form style={styles.form}>
      ...
      <button type="button" style={styles.button} onClick={() : void => submitHandler()}>
        Submit
      </button>
    </form>
  )
